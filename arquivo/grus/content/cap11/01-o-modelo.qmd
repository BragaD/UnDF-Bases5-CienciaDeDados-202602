...
len(num_friends_good), float(num_friends_good.mean()), float(daily_minutes_good.mean())
```
...
São 203 usuários. Em média, cada um tem quase 7 amigos e passa cerca de 29 minutos por dia no site. Como a [seção 7.1](../cap07/01-explorando-seus-dados.qmd) mostrou, `np.corrcoef` devolve a matriz 2 × 2 de correlações, e o elemento fora da diagonal, `[0, 1]`, é o que interessa — é assim que o resto da seção vai escrevê-la. Pouco mais de 0,57 diz que existe uma tendência real: mais amigos, mais tempo. Mas repare no que esse número **não** diz. Ele não diz se um amigo a mais vale, em média, 10 segundos ou 10 minutos a mais de uso. Ele não dá uma equação. Para isso, precisamos de outra coisa: um modelo.
...
len(num_friends), float(np.corrcoef(num_friends, daily_minutes)[0, 1])
```
...
Três operações, nenhum laço: `error(alpha, beta, x, y)` devolve os 203 erros de uma vez, `** 2` eleva cada um ao quadrado e `np.sum` reduz tudo a um número. O `float()` no fim troca o escalar do `numpy` por um número do Python — só para a saída na tela ser um número do Python — `13196.6...`, quando a seção 11.3 o imprimir —, e não `np.float64(...)`.
...
O `ddof=1` pede o desvio padrão **amostral**, o de denominador $n - 1$ que a [seção 7.6](../cap07/06-reescalonamento.qmd) apresentou — a mesma fórmula que o livro-texto usa. Aqui ele quase não muda o resultado: `beta` é uma **razão** de desvios padrão, e matematicamente o denominador cancela, seja ele $n$ ou $n - 1$ — na prática só o último bit muda. Escrevê-lo mesmo assim mantém a linha igual à fórmula que ela representa, e evita que alguém a copie para um lugar onde o cancelamento não aconteça.
...
Repare no que **não** há aqui: nenhum laço nem compreensão de lista para montar `X` — a biblioteca pede a mesma convenção deste livro, uma matriz `(n, d)`, e o `reshape(-1, 1)` só diz que os 203 números são 203 linhas de uma coluna.
