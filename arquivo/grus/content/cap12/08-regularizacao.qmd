...
Na regressão *ridge*, a penalidade é proporcional à soma dos quadrados dos coeficientes — com uma exceção importante, o termo constante. `beta[1:] @ beta[1:]` é a soma dos quadrados dos coeficientes a partir do segundo: o mesmo `@` do produto escalar, com o vetor multiplicado por ele mesmo.
...
Como a perda é uma soma de duas parcelas, o gradiente é a soma dos dois gradientes — e com arrays isso é o `+` de sempre. O `np.concatenate` na linha de cima é o `[0.] + [...]` do vetor: um zero na frente, porque o termo constante não é penalizado, e `2 * alpha * beta[1:]` — a derivada de $\alpha \beta_j^2$ — no resto.

`error` e `sqerror_gradient`, usados acima, são exatamente as funções da [seção 12.3](03-ajustando-o-modelo.qmd), importadas de `scratch_np.multiple_regression`. A partir daí, a mudança conceitual no otimizador é uma só: trocar `sqerror_gradient` por `sqerror_ridge_gradient`. Na versão em arrays isso aparece de um jeito ainda mais econômico — a penalidade **não depende do ponto**, então a média dela sobre o lote é ela mesma, e ela entra somada uma vez só ao gradiente do erro. O chute inicial, o laço sobre lotes, o `gradient_step` — tudo o que faz o gradiente descendente ser gradiente descendente fica igual. O pacote `scratch_np` já traz essa versão pronta, como `least_squares_fit_ridge`.

## O efeito de apertar a penalidade

Com `alpha = 0` não há penalidade nenhuma, e devemos recuperar o ajuste da seção 12.3. Conforme `alpha` cresce, o ajuste piora e os coeficientes encolhem. O gerador é recriado a cada linha, com a mesma semente: assim a única coisa que muda entre elas é o `alpha`.
