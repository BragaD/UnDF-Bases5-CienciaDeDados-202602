...
propaganda = pd.read_csv("dados/Advertising.csv", index_col=0)
propaganda.shape, propaganda.columns.tolist()
```

O capítulo 6 já ensinou a conferir o que um arquivo traz antes de confiar nele: sem `index_col=0`, a primeira coluna chegaria como `Unnamed: 0`, sobra do índice de linha que o R grava ao exportar — não é variável nenhuma, é um acidente do formato. Corrigido, restam duzentos mercados e quatro colunas: `TV`, `radio` e `newspaper`, o investimento em cada mídia, e `sales`, o volume vendido.
...
estudo_renda = pd.read_csv("dados/Income1.csv", index_col=0)
estudo_renda.shape, estudo_renda.columns.tolist()
```

Trinta indivíduos, anos de estudo (`Education`) e renda em milhares de dólares (`Income`) — de novo a coluna de índice do R descartada com `index_col=0`.
...
educacao = estudo_renda["Education"].to_numpy()
renda_observada = estudo_renda["Income"].to_numpy()
f_suave = estudo_renda["Income"].rolling(window=13, center=True, min_periods=1).mean().to_numpy()
