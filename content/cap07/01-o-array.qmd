...
O capítulo anterior separou `alugueis` em `X`, os preditores, e `y`, o alvo — os dois como objetos do `pandas`:
...
`.to_numpy()` devolve o que está por baixo dessa tabela: só os números, num array só.

```{python}
#| label: dataframe-para-array
X_array = X.to_numpy()
type(X_array), X_array.shape, X_array.dtype
```

Mesma forma, `(10692, 4)`, mas o `DataFrame` virou `ndarray`, e junto com o tipo foram embora os nomes de coluna. A primeira linha de `X_array`,
