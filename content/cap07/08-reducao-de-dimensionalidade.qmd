...
Especificamente, dada uma direção $d$ (um vetor de magnitude 1), cada linha $x$ da matriz se estende `dot(x, d)` na direção $d$. E todo vetor $w$ não nulo determina uma direção, se o reescalonarmos para ter magnitude 1:
...
#| fig-cap: "Primeiro componente principal"
escala = 30
plt.scatter(xs2, ys2, s=15, color="darkblue")
...
#| fig-cap: "Dados depois de remover o primeiro componente principal"
xs3 = [p[0] for p in residual]
ys3 = [p[1] for p in residual]
plt.scatter(xs3, ys3, s=15, color="darkblue")
plt.title("Dados depois de remover o primeiro componente principal")
plt.show()
```

Os pontos ainda têm duas coordenadas, mas agora estão essencialmente alinhados ao longo de uma reta — quase toda a variação que sobrou está numa única direção, a segunda componente principal.
...
Com as duas componentes deste exemplo bidimensional, `transform` só reexpressa cada ponto num novo par de eixos — não reduz nada. O ganho aparece quando `num_components` é bem menor que o número de dimensões originais: um conjunto de centenas de colunas pode virar um punhado de componentes que ainda capturam a maior parte da variação.
