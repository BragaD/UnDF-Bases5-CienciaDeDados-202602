...
::: {.callout-note}
O comentário diz "palpite aleatório", mas repare no código: `guess` começa sempre em `[1.0, 1.0]`. Não há aleatoriedade nenhuma aqui — o gradiente parte sempre do mesmo lugar e segue sempre o mesmo caminho, então o resultado é determinístico e os dois `assert` acima valem em qualquer execução, sem precisar de `random.seed`. Uma inicialização genuinamente aleatória — como a que o [Capítulo 15](../cap15/index.qmd) usa para os pesos de uma rede neural — exigiria a semente para valer o mesmo.
:::
