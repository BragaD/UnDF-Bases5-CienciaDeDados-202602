...
Considere dois conjuntos de dados fictícios: um uniforme entre -100 e 100, e um normal com média 0 e desvio padrão 57. Para o segundo, precisamos da função inversa da normal acumulada, que veio do [Capítulo 6](../cap06/index.qmd)... não — ela na verdade nunca apareceu em nenhum capítulo deste livro até aqui, porque vive em `scratch/probability.py`, um módulo fora do escopo desta disciplina (o tratamento de probabilidade do Grus não entrou na ementa), mas que o próprio pacote do livro-texto usa como dependência. Importamos só a função:
...
Os dois têm média próxima de zero e desvio padrão próximo de 57. Ainda assim, um é achatado e limitado; o outro tem pico ao centro e caudas compridas. Nenhuma estatística resumo diz isso — só olhar para a forma diz.
...
A matriz confirma o que os gráficos sugerem: a série 1 é fortemente **anticorrelacionada** com a série 0 (a própria fórmula de `row[1]` inverte o sinal de `row[0]`); a série 2 é positivamente correlacionada com a série 1; e a série 3 só assume os valores 0 e 6 — 6 quando a série 2 é grande, 0 quando é pequena —, o que explica as colunas de pontos verticais nos gráficos que envolvem ela.
