...
::: {.callout-note}
O `import` acima roda o módulo `scratch/probability.py` inteiro, que tem 18 chamadas `plt.*` e vários `assert` no corpo do arquivo (documentado no `CLAUDE.md` deste repositório). Os `assert` são sobre dado fixo e sempre passam; os gráficos, ninguém pediu para ver — por isso o chunk é `#| include: false`, e `plt.close('all')` limpa qualquer figura que o import tenha deixado pendurada, para que ela não vaze para a próxima célula.
:::
...
O código-fonte de `working_with_data.py` (linhas 44 a 49 do @grus2019) gera `xs`, `ys1` e `ys2` exatamente como acima, mas **sem fixar semente** — e na sequência afirma:
