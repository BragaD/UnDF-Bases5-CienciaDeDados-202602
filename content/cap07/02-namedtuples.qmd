...
O bloco *De listas a arrays* da [seção 7.1](01-explorando-seus-dados.qmd) deixou claro o que um array não faz: guardar um `str`, uma data e um `float` lado a lado — o `dtype` é um só. Um registro heterogêneo pede outra estrutura, e a forma mais comum de representá-lo é um `dict`:
...
`NamedTuple` resolve o problema do registro heterogêneo tipado, mas continua imutável — o que é uma escolha, não uma limitação a se contornar. Mutabilidade, sem abrir mão da anotação de tipo, é o assunto da [próxima seção](03-dataclasses.qmd): `dataclass` — que fecha apontando para um terceiro caminho, o de quando os registros não vêm um por vez, mas aos milhares.
:::
