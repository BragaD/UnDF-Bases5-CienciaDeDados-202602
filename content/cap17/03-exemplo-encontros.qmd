...
def desenha_grupos(grupos: List[List[Vector]], titulo: str) -> None:
    for i, (grupo, marca, cor) in enumerate(zip(grupos, ['D', 'o', '*'],
        ...
        # e um número no centro do grupo
...
Comparando as duas figuras, o grupo da direita ficou intacto e os dois da esquerda viraram um só. Dá para conferir isso sem depender do olho, verificando se cada grupo da solução com *k* = 3 cabe inteiro dentro de algum grupo da solução com *k* = 2:
...
Os centros não saem idênticos aos nossos, e a razão é a mesma de sempre neste livro: a inicialização é diferente. O `scikit-learn` usa `k-means++` e roda o algoritmo dez vezes por padrão, ficando com o melhor resultado — enquanto o nosso `train` sorteia uma atribuição inicial uniforme e aceita o primeiro ótimo local que encontrar.
