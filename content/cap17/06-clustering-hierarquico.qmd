...
#| fig-cap: "Três grupos bottom-up com ligação mínima. O grupo dos diamantes vermelhos tem catorze dos vinte usuários."
...
Esta é a mesma divisão que o k-means encontrou com *k* = 3. Não "parecida": a mesma. Dá para verificar comparando as duas partições ponto a ponto:
...
O `bottom_up_cluster` remove os dois grupos fundidos com `[c for c in clusters if c != c1 and c != c2]`, e `c != c1` compara `NamedTuple`s **por valor**. Duas folhas com as mesmas coordenadas são, para o Python, o mesmo objeto.
