...
Esse primeiro ponto merece ser dito com todas as letras, porque é a novidade do capítulo. Você já construiu quatro classificadores neste livro, e nenhum deles se explicava:
...
Uma árvore de decisão sozinha raramente é o modelo final de um sistema em produção — ela é frágil demais, pelo motivo que a [seção 14.6](06-florestas-aleatorias.qmd) vai medir. O que domina hoje o aprendizado sobre dados tabulares são os **comitês de árvores**: florestas aleatórias e, sobretudo, *gradient boosted trees* (`XGBoost`, `LightGBM`, `HistGradientBoostingClassifier` do `scikit-learn`). Em dados em formato de tabela — a maior parte do que existe numa empresa —, esses métodos costumam bater redes neurais, e por margem confortável.
...
Um aviso para quando você for comparar com a biblioteca: a árvore do `scikit-learn` **não** é a deste capítulo. `DecisionTreeClassifier` implementa uma variante do CART, que faz sempre divisões **binárias** sobre atributos **numéricos** — `x[3] <= 2.5` —, e por isso exige que atributos categóricos sejam convertidos a número antes. O ID3 que vamos escrever faz uma divisão com **um ramo por valor** de um atributo categórico, que é o que torna a árvore do vice-presidente de Talentos legível do jeito que ela é.
:::
