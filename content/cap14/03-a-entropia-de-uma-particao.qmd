...
def painel(ax, dados, atributos, titulo):
    ...
    ax.text(0.98, 0.94, f"antes: {antes:.3f}".replace('.', ','), color='tab:red',
    ...
    for b, h in zip(barras, hs):
        venceu = h == min(hs)
        ax.text(h + 0.02, b.get_y() + b.get_height() / 2,
                f"{h:.3f}".replace('.', ','), va='center', fontsize=8,
                color='tab:green' if venceu else 'black',
