...
::: {.callout-note}
## Por que o `import` está escondido
...
Agora o ajuste, com o `least_squares_fit` do capítulo anterior, sem uma linha de mudança:
...
::: {.callout-note}
O `#| warning: false` é obrigatório em qualquer chunk que chame `least_squares_fit`: a função usa `tqdm.trange` por dentro, o `tqdm` escreve em `stderr`, e o Quarto captura `stderr` como aviso. Sem essa opção, dezenas de linhas de barra de progresso apareceriam aqui. A [seção 12.3](../cap12/03-ajustando-o-modelo.qmd) discute o julgamento por trás disso.
:::
...
O primeiro salta do gráfico. As duas linhas pontilhadas marcam 0 e 1, os únicos valores que a variável a prever assume. Boa parte dos pontos está fora delas:
