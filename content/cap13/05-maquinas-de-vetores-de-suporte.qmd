...
A fronteira é `beta_unscaled[0] + beta_unscaled[1] * experiência + beta_unscaled[2] * salário = 0`. Isolando o salário, ela vira uma reta no plano do gráfico da [seção 13.1](01-o-problema.qmd) — e `salario_na_fronteira` recebe a `grade` inteira de uma vez, em vez de ser chamada uma vez por ponto:
...
`np.linalg.norm` é o comprimento do vetor — a função `magnitude` do [Capítulo 4](../cap04/01-vetores.qmd), que era `math.sqrt(sum_of_squares(v))`, numa chamada só —, e o `rng_sep` é um gerador próprio porque este conjunto é inventado aqui e não tem nada a ver com a divisão treino/teste do resto do capítulo.
