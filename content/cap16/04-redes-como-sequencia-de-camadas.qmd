...
Repare no detalhe: o mesmo objeto `bloco` aparece duas vezes na lista, e por isso os pesos dele aparecem **duplicados** em `params()` — o gerador percorre a lista de camadas, não um conjunto de objetos distintos. Um otimizador aplicaria o passo àquele tensor duas vezes por exemplo, com dois gradientes diferentes, e o segundo sobrescreveria o `w_grad` do primeiro antes de ser usado.
...
## Na prática: `torch.nn.Sequential`
...
Mesmo nome de classe, mesma ideia, mesma lista de camadas. O Keras tem `keras.Sequential([...])`, com a mesma assinatura — e é dele que o desenho da biblioteca do livro-texto é vagamente copiado, como a seção de leituras adicionais do capítulo registra.
