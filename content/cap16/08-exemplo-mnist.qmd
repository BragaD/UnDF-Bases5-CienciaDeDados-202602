...
Isso é o assunto, não um contratempo. **A lentidão é o preço da transparência**, e este capítulo é o único do livro em que esse preço tem unidade e valor medido. Uma biblioteca vetorizada rodando numa GPU faz esta mesma rede, sobre o conjunto inteiro, em segundos — e é exatamente por isso que GPUs existem. Um aluno que só chamou `.fit()` nunca teve como sentir a diferença entre "seis minutos" e "seis segundos" como uma consequência de uma escolha de representação.
...
**A rede profunda supera a regressão logística — por pouco.** Ao fim do terceiro epoch ela chega perto de 88% no teste, contra os cerca de 86% do modelo linear. Vale dizer com franqueza: são 2.000 imagens de teste, e uma diferença de dois pontos percentuais está no limite do que essa amostra distingue. O que a tabela mostra com segurança é a **tendência** — a rede profunda ainda estava melhorando quando o orçamento acabou, e o modelo linear já não estava.

**O custo por imagem triplicou.** Cerca de 6 milissegundos contra os 2 do modelo linear, para três vezes mais parâmetros. É proporcional, e é o que se espera: quase todo o tempo é gasto nas multiplicações da camada de 784 × 30.
