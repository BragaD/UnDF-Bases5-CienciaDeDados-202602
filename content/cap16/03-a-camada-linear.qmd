...
::: {.callout-note}
O `Layer` e o `Tensor` vêm de `scratch.deep_learning` — são o mesmo código da [seção 16.2](02-a-abstracao-de-camada.qmd), não uma segunda versão dele. O `inverse_normal_cdf` vem de `scratch/probability.py`.
:::
...
Os pesos saem cerca de **vinte vezes mais estreitos** do que a fórmula de Xavier pediria. Não vamos consertar isso: as redes deste capítulo treinam assim, com estes valores. Registre como aviso de leitura: quando este código diz "inicialização de Xavier", o que ele faz não é bem isso — e a observação de não se conseguir treinar essas redes com outras inicializações provavelmente tem a ver com a discrepância.
:::
...
::: {.exemplo}
## Por que essa conferência importa numa rede de várias camadas
