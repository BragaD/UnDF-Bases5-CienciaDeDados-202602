...
Essa é a justificativa do livro-texto, e vale dizer desde já que ela não é a nossa: **as redes deste capítulo nunca passam de duas dimensões.** As imagens do MNIST da [seção 16.8](08-exemplo-mnist.qmd) chegam achatadas em 784 números, e o maior objeto que qualquer página deste capítulo manipula é uma matriz. (Um arranjo de três dimensões aparece uma vez no livro, na [seção 17.5](../cap17/05-exemplo-clustering-de-cores.qmd) — uma imagem colorida, altura × largura × canais —, e a primeira coisa que aquela seção faz é achatá-lo.) A generalidade ainda paga — só que por outro motivo, que aparece no fim desta seção.
