...
Essa é a justificativa do livro-texto, e vale dizer desde já que ela não é a nossa: **as redes deste capítulo nunca passam de duas dimensões.** As imagens do MNIST da [seção 16.8](08-exemplo-mnist.qmd) chegam achatadas em 784 números, e todo tensor que entra ou sai de uma camada aqui é um vetor ou uma matriz. Arranjos de três dimensões aparecem no livro — a [seção 16.3](03-a-camada-linear.qmd) constrói um para testar a `shape`, e a [seção 17.5](../cap17/05-exemplo-clustering-de-cores.qmd) lê uma imagem colorida como altura × largura × canais —, mas nenhum deles atravessa uma rede. A generalidade ainda paga — só que por outro motivo, que aparece no fim desta seção.
