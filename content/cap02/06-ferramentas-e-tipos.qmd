...
Que você conhece aleatoriedade pseudoaleatória, expressões regulares e funções de alta ordem. **O que vale desacelerar: `zip` e, sobretudo, anotações de tipo** — que atravessam dois terços das funções do livro e são a contribuição mais distintiva do @grus2019.
:::
...
::: {.callout-important}
## A semente não é opcional neste livro

`random.seed(10)` faz o gerador produzir sempre a mesma sequência. Sem ela, cada execução dá números diferentes.

Neste material isso é **obrigatório em todo chunk que usa aleatoriedade**, e por uma razão prática: o livro é renderizado a cada mudança, e sem semente cada renderização produziria números e gráficos diferentes. O texto pararia de bater com a saída, e o diff do site publicado viraria ruído.

Você vai ver `random.seed(...)` em todos os capítulos que amostram, embaralham ou inicializam pesos ao acaso. Repare que é o `random` da biblioteca padrão, não o `numpy.random` — este livro é Python puro.
:::
...
## `zip` e desempacotamento — 60 usos no livro
...
**2. Ferramentas externas verificam.** O `mypy` lê o código, inspeciona as anotações e acusa erros de tipo **antes de você executar**. Como o `assert`, é uma forma de achar erro cedo. O autor conta que roda o mypy sobre o livro inteiro nos bastidores — é parte do que garante que o código publicado esteja correto.
