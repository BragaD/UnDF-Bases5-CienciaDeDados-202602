...
::: {.callout-important}
## Neste livro, o ambiente já está resolvido — e travado

Você não precisa criar ambiente nenhum para acompanhar este material. Tudo roda dentro de um container Docker que já traz o Python, o Quarto e todas as bibliotecas nas versões exatas.

Isso não é conveniência: é **reprodutibilidade**. As versões estão travadas num arquivo de lock, e o motivo é concreto. Numa disciplina irmã, uma atualização do pandas mudou silenciosamente o comportamento de uma comparação e passou a devolver a resposta errada em dois exemplos — sem erro, sem aviso, apenas o número errado no material publicado.

Se você quiser rodar os exemplos fora do container, use um ambiente virtual. Nunca instale as dependências deste livro no Python do sistema.
:::
...
::: {.callout-warning}
## Uma forma que o livro nunca usa

Existe uma quarta forma, que importa tudo para o espaço de nomes atual:
...
O código deste livro usa `import módulo` ou `from módulo import nome`, sempre listando o que traz.
:::
