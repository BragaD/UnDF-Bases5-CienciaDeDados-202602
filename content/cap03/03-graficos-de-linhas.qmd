...
E, para ilustrar isso, o livro-texto usa um exemplo que o [Capítulo 8](../cap08/05-vies-e-variancia.qmd) vai desenvolver: o compromisso entre viés e variância. Aqui ele serve só como forma — três curvas que contam uma história —; lá ele volta como conceito, construído do zero.
...
E note que os dois eixos não têm unidade: `plt.xticks([])` remove os rótulos do eixo x de propósito. O gráfico não afirma quanto vale a complexidade; ele afirma a **forma** da relação.
:::

## Séries múltiplas e legenda

O padrão que a seção introduz é o de chamar `plt.plot` várias vezes antes de `plt.show()`. Cada chamada acrescenta uma série ao mesmo gráfico.
