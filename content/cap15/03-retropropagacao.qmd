...
que é exatamente a linha `hidden_deltas` do código, com $v_{ih}$ escrito como `n[i]`. E, de novo, a derivada em relação a um peso da primeira camada é essa culpa vezes a entrada correspondente, $x_j$.
:::
...
Vinte mil épocas sobre quatro exemplos custam menos de um segundo. O `tqdm.trange` desenha a barra de progresso do [Capítulo 7](../cap07/07-um-parenteses-tqdm.qmd), e o chunk leva `#| warning: false` para que a barra não vaze para a página — ela é útil no terminal, ilegível num livro.
