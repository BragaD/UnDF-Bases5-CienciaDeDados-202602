...
No perceptron, o neurônio somava os produtos das entradas pelos pesos e aplicava a `step_function` ao resultado. Numa rede, vamos aplicar em vez dela uma **aproximação suave** do degrau. O motivo não é estético.
...
#| fig-cap: "À esquerda, o degrau e a sua aproximação suave. À direita, as derivadas das duas."
#| echo: false
...
fig, (esq, dir) = plt.subplots(1, 2, figsize=(10.5, 3.8))
...
dir.plot([x for x in grade if x < 0], [0.0 for x in grade if x < 0],
         color='0.35', label='derivada do degrau')
dir.plot([x for x in grade if x > 0], [0.0 for x in grade if x > 0],
         color='0.35')
dir.plot(grade, [sigmoid(x) * (1 - sigmoid(x)) for x in grade],
         color='C0', label='derivada da sigmoide')
dir.scatter([0], [0], s=60, facecolors='white', edgecolors='0.35',
            zorder=3, linewidths=1.6)
dir.annotate("indefinida", xy=(0, 0), xytext=(1.1, 0.09),
             fontsize=9, color='0.35',
             arrowprops=dict(arrowstyle='->', color='0.35'))
dir.set_title("as derivadas")
dir.set_ylim(-0.05, 0.3)
dir.legend(loc='upper right', prop={'size': 8})
