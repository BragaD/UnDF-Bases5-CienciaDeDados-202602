...
O reflexo depois de ver uma coluna com nulo é `dropna()`. Aqui ele erraria a pergunta: os anúncios desta base cobrem tanto apartamento quanto casa, e uma casa não tem andar — não porque o dado faltou, mas porque a pergunta "em que andar fica" não se aplica a um imóvel térreo por definição. O traço no arquivo original não era um vazio a preencher: era a resposta certa, escrita de um jeito que o `pandas` não reconhecia como número.
...
358 linhas repetem, em todas as 13 colunas, uma linha que já apareceu antes. Contando também as primeiras ocorrências de cada grupo repetido:
...
604 linhas — mais que o dobro de 358 —, o que já diz que alguns grupos têm mais de duas cópias, não só pares. Um exemplo:
...
O máximo de `area_m2` é 515 vezes a mediana da própria coluna; o de `condominio`, 1.995 vezes. E o topo de `area_m2` não é um ponto isolado, são três:

```{python}
#| label: outliers-area-top3
alugueis["area_m2"].sort_values(ascending=False).head(3).tolist()
```

46.335, 24.606 e 12.732 m² — todos acima de dez mil metros quadrados, muito à frente do quarto maior, que já cai para 2.000 m². Em `condominio`, o maior valor não é um ponto isolado no outro sentido: ele se repete.
