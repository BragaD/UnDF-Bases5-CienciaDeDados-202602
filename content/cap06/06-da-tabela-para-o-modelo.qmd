...
`pd.get_dummies` evita essa invenção: em vez de uma coluna com números inventados, cria uma coluna por categoria, cada uma com `True` onde a linha é daquela categoria e `False` onde não é.
...
::: {.funcao}
**`pd.get_dummies(serie)`** — cria uma coluna para cada valor distinto de `serie`, com `True` nas linhas que têm aquele valor e `False` nas demais. Em conta, `True` vale 1 e `False` vale 0.

- `serie`: a coluna categórica, aqui `alugueis["cidade"]`.
:::
...
::: {.funcao}
**`df.sum(axis=1)`** — `axis` escolhe a direção da soma. Com `0`, o padrão, soma cada coluna descendo pelas linhas; com `1`, soma cada linha atravessando as colunas.
:::
...
::: {.funcao}
**`pd.concat(tabelas, axis=1)`** — cola tabelas numa só.

- `tabelas`: uma lista com as tabelas a colar.
- `axis`: com `1`, lado a lado, casando as linhas pelo rótulo do índice; com `0`, o padrão, uma embaixo da outra.
:::

`X` passa de quatro para nove colunas — as quatro numéricas mais as cinco de `True`/`False` que substituem `cidade`, sem nenhuma ordem que a coluna original não tinha.
...
::: {.funcao}
**`serie.min()`** — o menor valor da coluna. Não recebe argumento; valores nulos são ignorados.
:::
...
::: {.funcao}
**`df.corr()`** — numa tabela, e sem argumento, a correlação de Pearson de cada coluna com cada outra. Devolve uma matriz quadrada, com as mesmas colunas nas linhas e nas colunas, e 1 na diagonal.

**`ax.imshow(matriz, cmap, vmin, vmax)`** — pinta cada célula da matriz com uma cor.

- `cmap`: a escala de cores, aqui `"Blues"`.
- `vmin`, `vmax`: os valores que recebem as duas pontas da escala.

**`ax.set_xticks(posicoes)`** e **`ax.set_xticklabels(rotulos, rotation, ha)`** — onde ficam as marcas do eixo horizontal e o texto de cada uma. `rotation` inclina o texto, em graus; `ha="right"` alinha o texto pela ponta direita, para que o rótulo inclinado termine embaixo da sua marca. `set_yticks` e `set_yticklabels` fazem o mesmo no eixo vertical.

**`ax.grid(False)`** — desliga a grade, que cortaria as células ao meio.

**`df.iloc[i, j]`** — com dois números, a célula na linha `i` e na coluna `j`, contadas por posição.

**`fig.colorbar(im, ax, label)`** — a barra que traduz cor em valor. `im` é o que `imshow` devolveu, `ax` é o painel ao lado do qual ela fica, e `label` é o título dela.
:::
...
```{python}
#| label: corr-sem-duas-linhas-outlier
...
```{python}
#| label: corr-0-1-porcento-condominio
...
::: {.funcao}
**`mascara1 & mascara2`** — o operador `&` combina duas colunas de `True`/`False` elemento a elemento: dá `True` só onde as duas são `True`. Os parênteses em volta de cada comparação são obrigatórios.
:::
