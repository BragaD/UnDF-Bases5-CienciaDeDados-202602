...
O que explica os 0,2645 está na própria matriz: `total` correlaciona 0,96 com `condominio`, não com `aluguel`. `condominio` é a mesma coluna cujo outlier a seção anterior nomeou — as duas linhas de condomínio R$ 1.117.000 — e manteve dentro de `alugueis`. Removendo só essas duas linhas:
...
Duas linhas, de 10.692, e a correlação entre `total` e `aluguel` sobe de 0,2645 para 0,706. Alargando o corte para o 0,1% de maior `condominio` — que inclui essas duas linhas mais outros imóveis de condomínio alto que a seção anterior não chegou a nomear:
