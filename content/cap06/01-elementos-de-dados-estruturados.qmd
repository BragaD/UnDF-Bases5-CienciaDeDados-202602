...
::: {.funcao}
**`serie.sum()`** — soma os valores da coluna. `faixa < "6 a 15 milhões"` devolve uma coluna de `True`/`False`, e numa soma cada `True` vale 1 e cada `False` vale 0: somar é contar quantos `True` existem.
:::
...
**`ax.hist(valores, bins)`** — histograma: divide o intervalo de `valores` em `bins` faixas de mesma largura e desenha quantos valores caem em cada uma.
