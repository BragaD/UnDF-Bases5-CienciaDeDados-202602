...
::: {.funcao}
**`agg(funcoes)`** — depois de um `groupby`, aplica várias funções de resumo de uma vez.

- `funcoes`: uma lista de nomes; `["median", "mean"]` produz uma coluna para cada função, com o nome dela.

**`df.round(casas)`** — o método arredonda todos os valores numéricos da tabela para `casas` decimais. É diferente do `round` do Python, que recebe um número só.

**`serie.all()`** — `True` se todos os valores da coluna forem `True`, e `False` se ao menos um não for.
:::
...
::: {.funcao}
**`agg(nome=(coluna, funcao))`** — a agregação nomeada: cada argumento cria uma coluna no resultado.

- `nome`: o nome da coluna criada, como `contagem` ou `aluguel_mediano`.
- `coluna`: a coluna da tabela que é resumida.
- `funcao`: o resumo aplicado em cada grupo; `"count"` conta os valores não nulos.
:::
...
::: {.funcao}
**`df.merge(outra, on)`** — junta duas tabelas, casando as linhas que têm o mesmo valor na coluna de ligação. As colunas de `outra` são acrescentadas às de `df`.

- `outra`: a tabela trazida, aqui `cidades`.
- `on`: o nome da coluna de ligação, que precisa existir nas duas tabelas.

Sem o parâmetro `how`, a junção é `"inner"`: uma linha sem par do outro lado fica fora do resultado.
:::
...
::: {.funcao}
**`df.drop_duplicates()`** — devolve uma nova tabela sem as linhas repetidas, mantendo a primeira ocorrência de cada uma.
:::
