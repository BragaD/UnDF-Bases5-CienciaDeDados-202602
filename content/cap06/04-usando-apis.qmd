...
::: {.callout-important}
## Por que a busca no GitHub não roda aqui

`requests.get(endpoint)` é uma requisição de verdade, e diferente da seção anterior não há aqui um arquivo vendorizado para substituí-la: a resposta é a lista de repositórios da conta `joelgrus` **no momento em que a requisição roda**. Fixar uma cópia antiga do resultado seria fingir uma resposta que a API já não dá mais.
:::

O que vem depois de `repos` existir, porém, é Python puro sobre uma lista de `dict`s — nenhuma linha daqui para baixo toca rede. As datas na resposta chegam como strings, por exemplo `"created_at": "2013-07-05T02:02:28Z"`, e Python não vem com um bom parser de datas embutido; por isso este projeto instala o `python-dateutil`, do qual você provavelmente só vai precisar da função `dateutil.parser.parse`.

Para ver esse processamento rodando de verdade, eis um `repos` inventado, no mesmo formato que a API devolveria — os cinco repositórios (fictícios) do time de engenharia da DataSciencester:
...
Para uma API específica, vale checar se já existe uma biblioteca Python pronta antes de montar as chamadas na mão: sites como Yelp, Instagram e Spotify têm as suas próprias, e o [Real Python](https://realpython.com/), um site de tutoriais Python, mantém uma lista de *wrappers* conhecidos. Quando não existe nenhuma, sempre resta a raspagem — o último recurso do cientista de dados, coberto na [seção anterior](03-raspando-a-web.qmd).
