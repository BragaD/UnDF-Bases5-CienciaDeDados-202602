...
O coeficiente de `num_amigos` vai de cerca de 0,84 no modelo de uma variável só a cerca de 0,97 no modelo completo — uma diferença de mais de 15%. Não é que um deles esteja "errado" e o outro "certo": cada um responde a uma pergunta diferente, e a [seção 12.4](04-interpretando-o-modelo.qmd) é sobre qual pergunta é essa.

::: {.callout-warning}
## O número de 0,84 e o número de 0,904

O [Capítulo 11](../cap11/01-o-modelo.qmd) ajustou exatamente este primeiro modelo — só `amigos` — pela fórmula fechada, e encontrou $\beta \approx 0{,}9039$. Aqui o mesmo modelo, ajustado por gradiente descendente com estes hiperparâmetros, dá cerca de 0,84.

A diferença é do **otimizador**, não do modelo. O `least_squares_fit` usado aqui dá 5.000 passos com lotes de 25 pontos e taxa de aprendizado fixa; ele chega perto do mínimo e fica orbitando em torno dele, sem nunca pousar. Para o modelo de quatro variáveis, que é o deste capítulo, essa imprecisão é pequena (a [seção 12.3](03-ajustando-o-modelo.qmd) compara com a solução exata); para o modelo de duas, com estes mesmos hiperparâmetros, ela é maior.

Vale registrar isso em vez de esconder, por dois motivos. Primeiro, porque o **padrão** — o coeficiente de `amigos` subir quando as outras variáveis entram — é robusto e aparece igual na solução exata (0,904 → 0,972). Segundo, porque é um lembrete de que todo número deste capítulo tem duas fontes de incerteza empilhadas: a dos dados e a do otimizador. As seções 12.6 e 12.7 medem a primeira. A segunda se combate com mais passos, e é por isso que os chunks deste capítulo pedem 5.000 deles.
:::
