...
O pacote vendorizado do livro-texto já traz as 203 linhas prontas, no mesmo formato:
...
::: {.callout-important}
## Por que o `import` está escondido, e por que ele leva uma semente

O chunk que importa `scratch.multiple_regression` está marcado com `#| include: false`, por dois motivos que valem para vários capítulos deste livro.

O primeiro já apareceu no [Capítulo 11](../cap11/01-o-modelo.qmd): `scratch/statistics.py` e `scratch/probability.py` têm chamadas `plt.*` soltas no nível do módulo — cinco e dezoito, respectivamente — e `multiple_regression` importa os dois. Sem o `include: false` e o `plt.close('all')`, figuras que ninguém pediu apareceriam no meio da página.

O segundo é específico deste módulo: ele também executa, no nível do módulo, um trecho de reamostragem aleatória **sem semente**, com asserções sobre o resultado. É o código que a [seção 12.6](06-digressao-o-bootstrap.qmd) vai estudar. Rodar `random.seed(0)` **antes** do `import` torna esse trecho determinístico e o `import` reprodutível. A regra "toda aleatoriedade leva semente explícita" vale aqui exatamente como vale dentro de um chunk — a diferença é que aqui a aleatoriedade está escondida numa importação, que é o último lugar onde se pensa em procurá-la.

A correção fica no `.qmd`, nunca em `scratch/`: o pacote é uma cópia literal do repositório do livro-texto, e não se edita.
:::
