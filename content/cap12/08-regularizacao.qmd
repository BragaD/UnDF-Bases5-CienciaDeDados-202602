...
`error` e `sqerror_gradient`, usados acima, são exatamente as funções da [seção 12.3](03-ajustando-o-modelo.qmd); como cada página deste livro roda num kernel próprio, elas vêm importadas de `scratch.multiple_regression`, que é o mesmo código. A partir daí, a mudança conceitual no otimizador é uma só: trocar `sqerror_gradient` por `sqerror_ridge_gradient` dentro do `least_squares_fit` daquela seção. O chute inicial, o laço sobre lotes, o `gradient_step` — tudo o que faz o gradiente descendente ser gradiente descendente fica igual. O pacote vendorizado já traz essa versão pronta, como `least_squares_fit_ridge`. Se você abrir o arquivo para conferir, vai encontrar mais uma diferença além dessa, e ela não é conceitual: o `tqdm.trange` do laço externo virou um `range` comum, ou seja, esta versão simplesmente não desenha barra de progresso.
