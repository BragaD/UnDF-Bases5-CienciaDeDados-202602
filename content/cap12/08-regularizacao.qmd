...
# alpha é um *hiperparâmetro* que controla a dureza da penalidade.
...
::: {.exemplo}
Vale reparar que este é o segundo limite de escopo deste capítulo, e que os dois são de naturezas diferentes.
...
Um aviso de nomenclatura: o `alpha` do `Ridge` do `scikit-learn` tem o mesmo papel do nosso, mas o `LogisticRegression` usa `C`, que é o **inverso** da força da penalidade — ali, `C` pequeno significa penalidade forte. Confundir os dois inverte o modelo por completo, e é um erro que não gera exceção nenhuma.

O `LogisticRegression` aparece já no [Capítulo 13](../cap13/index.qmd), e vale registrar como este capítulo se conecta com ele: o próximo modelo troca a soma dos erros ao quadrado por outra função de perda, perde a fórmula fechada de vez e passa a depender inteiramente do gradiente descendente. A regularização desta seção o acompanha sem alteração conceitual — uma penalidade somada à perda, um gradiente somado ao gradiente —, e por isso o `scikit-learn` a mantém **ligada por padrão** naquele modelo.
:::
