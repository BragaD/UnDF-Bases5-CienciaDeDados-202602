...
for k in [0, 1, 5, 10, 15, 20]:
    xs = np.column_stack([inputs, ruido[:, :k]])
    rng = np.random.default_rng(0)
...
Vinte colunas de `rng_ruido.random(...) * 10`. Nenhuma delas sabe coisa alguma sobre quantos minutos por dia um usuário passa no site — são números sorteados, gerados sem olhar para `daily_minutes_good` uma única vez. Ainda assim, o R² subiu de 0,6800 para 0,7004.
