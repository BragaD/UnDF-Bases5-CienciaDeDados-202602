...
#| fig-cap: "Distribuição dos 100 coeficientes bootstrap. A linha tracejada marca o zero."
#| echo: false
...
for ax, i, titulo in zip(axes, [1, 2, 3],
    ...
    ax.hist(coluna, bins=20, color='tab:blue', edgecolor='white')
...
O gráfico mostra o que a tabela resume. As distribuições de `amigos` e de `horas de trabalho` estão inteiramente de um lado do zero — nenhuma das 100 reamostras produziu um coeficiente de sinal trocado. A de `doutorado` atravessa o zero: uma boa parte das reamostras produziu um coeficiente **negativo**.
...
Este resultado não deveria surpreender quem leu a [seção 12.2](02-hipoteses-do-minimos-quadrados.qmd). Lá medimos que `tem_doutorado` é quase determinado por `num_amigos`: abaixo de 6 amigos, o usuário tem doutorado; acima de 6, não tem. A única faixa em que os dois grupos coexistem é a de exatamente 6 amigos — 22 usuários dos 203.

Isso é multicolinearidade quase perfeita, e o erro padrão de 1,249 é exatamente a forma como ela se manifesta. O modelo precisaria comparar pessoas com o mesmo número de amigos e formação diferente para separar os dois efeitos, e essa comparação só existe naqueles 22 usuários. Cada reamostra bootstrap sorteia um punhado diferente deles, e o coeficiente balança junto.

::: {.callout-important}
Repare no arco que se fechou aqui. A [seção 12.2](02-hipoteses-do-minimos-quadrados.qmd) apontou uma propriedade estrutural dos dados. A [seção 12.4](04-interpretando-o-modelo.qmd) explicou por que ela torna a frase "mantendo tudo o mais constante" vazia para aquele coeficiente. A [seção 12.5](05-qualidade-do-ajuste.qmd) mostrou que o R² é cego para esse tipo de problema. E o bootstrap, sem saber nada de nada disso — sem teoria, sem álgebra, sem sequer olhar para a matriz de correlações —, mediu o efeito no número que interessa.
:::
...
A diferença de fundo: os erros padrão do `statsmodels` vêm da **fórmula fechada**, sob a hipótese de erros normais independentes com variância constante. Os nossos vieram do bootstrap, que não assume nada disso. Nestes dados os dois caminhos chegam perto: @grus2019 registra, ao lado dos valores bootstrap, os erros exatos calculados pela fórmula — 1,19 / 0,080 / 0,127 / 0,998, contra os nossos 1,272 / 0,103 / 0,155 / 1,249. O bootstrap com apenas 100 reamostras superestima um pouco, e mais reamostras aproximariam os dois.

O que interessa é o **padrão**, e ele é idêntico pelos dois caminhos: o erro do `doutorado` é uma ordem de grandeza maior que o de `amigos`, e maior que o próprio coeficiente. Quando os dois caminhos **não** concordam, geralmente é sinal de que alguma hipótese da fórmula não vale para aqueles dados — e aí é o bootstrap que merece mais crédito, porque ele nunca dependeu dela.
:::
