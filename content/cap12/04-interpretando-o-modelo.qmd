...
from scratch.statistics import daily_minutes_good, mean
...
com_doutorado = [a for a, p in zip(amigos, phd) if p == 1]
sem_doutorado = [a for a, p in zip(amigos, phd) if p == 0]

print(f"com doutorado: {min(com_doutorado)} a {max(com_doutorado)} amigos "
      f"(média {mean(com_doutorado):.2f})")
print(f"sem doutorado: {min(sem_doutorado)} a {max(sem_doutorado)} amigos "
      f"(média {mean(sem_doutorado):.2f})")

sobrepostos = sum(1 for a, p in zip(amigos, phd) if a == 6)
print(f"usuários na única faixa de amigos onde os dois grupos coexistem: {sobrepostos}")
```
