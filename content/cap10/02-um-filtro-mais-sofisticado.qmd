...
::: {.exemplo}
O exemplo a seguir é sintético, só para tornar o problema visível — não são números do classificador real. Imagine duas hipóteses concorrentes, cada uma resultando de multiplicar centenas de probabilidades pequenas. `np.full(300, 0.01)` cria um array de 300 posições, todas iguais a 0,01; `np.prod` multiplica todos os elementos de um array entre si — é a redução irmã do `np.sum` da [seção 7.1](../cap07/01-explorando-seus-dados.qmd), com produto no lugar da soma —, e `float()` traz o resultado de volta como número do Python, só para a saída sair limpa:
...
::: {.callout-note}
Este é o mesmo problema, e a mesma solução, do [capítulo sobre extração de atributos](../cap08/06-extracao-e-selecao-de-atributos.qmd): um atributo do tipo sim-ou-não (a palavra apareceu ou não) precisa de uma estimativa que nunca trave em 0 ou em 1 diante de pouco dado. Um pseudocontador tem o mesmo espírito de um *prior* fraco — uma estimativa inicial, antes de olhar qualquer dado, que os dados corrigem aos poucos conforme chegam.
:::
