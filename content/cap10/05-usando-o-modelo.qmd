...
O corpus SpamAssassin completo soma centenas de megabytes, espalhados por milhares de arquivos de e-mail. E o código logo acima descarta tudo, de cada arquivo, menos a linha `Subject:`. Baixar centenas de MB para usar uma linha de texto por arquivo não se justifica dentro de um repositório de livro — então este material já traz commitado só o que o classificador de fato usa: `dados/spam-assuntos.csv`, com duas colunas, `assunto` e `is_spam`.
...
`precision` divide `tp` por `tp + fp`, e as duas são zero — o classificador nunca aponta "spam", então não há nada para dividir. É o par 98,1%/1,4% do teste do Luke, agora com dados reais: acurácia enorme, revocação zero, precisão que não existe.
...
Nesta renderização: 80 verdadeiros positivos, 24 falsos positivos, 675 verdadeiros negativos e 46 falsos negativos. `accuracy`, `precision`, `recall` e `f1_score` são as mesmas funções do [Capítulo 8](../cap08/04-correcao.qmd) — importadas de `scratch.machine_learning` no chunk do piso, não redefinidas aqui. A acurácia sai em 91,5%, a precisão em 76,9%, a revocação em 63,5% e o F1 em 0,696.
...
Vale parar também num detalhe fácil de passar batido: `predict` devolve uma **probabilidade**, não um rótulo — e o `> 0.5` do chunk `treina-e-preve` é uma escolha nossa, não uma exigência do modelo. O [Capítulo 8](../cap08/04-correcao.qmd) já avisou que, num filtro de spam, um falso positivo (e-mail importante que some no lixo) costuma custar mais caro que um falso negativo (propaganda que passa), e que por isso o limiar deveria pender para a precisão — um limiar mais alto que 0,5, trocando alguns dos 46 falsos negativos por menos dos 24 falsos positivos. É também o contraste mais limpo com o k-vizinhos do Capítulo 9: lá, `majority_vote` devolve um rótulo, ponto final, sem botão para girar depois. Aqui há um botão, e girá-lo é o que você de fato vai fazer ao usar isto.

Os números da nossa renderização ficam muito perto dos que @grus2019 relata rodando o mesmo código sobre o corpus completo — 84 verdadeiros positivos, 25 falsos positivos, 703 verdadeiros negativos, 44 falsos negativos, com precisão de 77% e revocação de 65%. A proximidade não é coincidência: o CSV que carregamos veio do mesmo corpus, só reduzido antes, em vez de durante, a renderização — e é mais uma evidência de que manter só os assuntos preservou o que o classificador de fato usa.
...
@grus2019 fecha o capítulo com quatro sugestões de melhoria. Nenhuma delas é implementada aqui, mas vale registrar o que ficou de fora e por quê:
...
- Um **stemmer**: uma função que reduz palavras aparentadas à mesma forma, para que `cheap` e `cheapest` contem como o mesmo token. O próprio módulo vendorizado, `scratch/naive_bayes.py`, define um stemmer ingênuo que nenhuma seção deste capítulo chama:
  ...
  Ele existe no código do livro como esboço de uma ideia, não como parte do classificador — daí não aparecer em nenhum chunk executado aqui. Um stemmer sério é bem mais delicado que remover um "s" final; o [Porter stemmer](https://tartarus.org/martin/PorterStemmer/), citado pelo próprio @grus2019, é o padrão da área.
