...
Duas coisas cuidam disso. O `random.seed(0)` garante que o resultado é sempre o mesmo, e o `freeze: auto` do `_quarto.yml` — opção de *projeto*, por arquivo inteiro, não de chunk — guarda em `_freeze/` os resultados já calculados: a próxima renderização só reexecuta este `.qmd` se o código-fonte dele mudar; caso contrário, reaproveita a saída congelada. `cache: true` também existe como opção de chunk, mas depende do pacote opcional `jupyter-cache`, que este projeto não instala — por isso não aparece aqui: o `freeze` já cobre o que ele cobriria.
