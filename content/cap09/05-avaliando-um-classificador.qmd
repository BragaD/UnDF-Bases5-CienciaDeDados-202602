...
round(acerto_trivial, 4), round(erro_trivial, 4)
```
...
perdidos_so_cresce_com_limiar = bool((np.diff(taxa_perdidos) >= 0).all())
falso_alarme_so_cai_com_limiar = bool((np.diff(taxa_falso_alarme) <= 0).all())

indice_cruzamento = int(np.argmin(np.abs(taxa_perdidos - taxa_falso_alarme)))
limiar_cruzamento = float(grade_limiares[indice_cruzamento])

indice_erro_minimo = int(np.argmin(erro_total))
limiar_erro_minimo = float(grade_limiares[indice_erro_minimo])

(
    perdidos_so_cresce_com_limiar,
    falso_alarme_so_cai_com_limiar,
    round(limiar_cruzamento, 3),
    round(float(taxa_perdidos[indice_cruzamento]), 4),
    round(float(taxa_falso_alarme[indice_cruzamento]), 4),
    round(limiar_erro_minimo, 3),
    round(float(erro_total[indice_erro_minimo]), 4),
    round(float(erro_total[-1]), 4),
)
```

Nos 251 limiares varridos, a fração de inadimplentes perdidos nunca cai quando o limiar sobe (`perdidos_so_cresce_com_limiar` é `True`), e a fração de alarme falso entre adimplentes nunca sobe quando o limiar sobe (`falso_alarme_so_cai_com_limiar` é `True`): as duas trocam de direção o tempo todo, em sentidos opostos, não só nos três pontos escolhidos acima. As duas curvas se cruzam perto do limiar 0,038, onde ambas valem cerca de 0,12 — abaixo desse ponto, o modelo já erra mais por alarme falso do que ganha em achar inadimplente. E o erro total mínimo da varredura, 0,0261, acontece no limiar 0,428, não exatamente em 0,5 (que dá 0,0268): perto de 0,5 o erro total está quase plano, não estritamente mínimo ali.
...
ax.scatter([limiar_cruzamento], [taxa_perdidos[indice_cruzamento]], color="#6C757D", zorder=5, s=28)
ax.annotate(
    f"limiar ≈ {limiar_cruzamento:.3f}",
    xy=(limiar_cruzamento, taxa_perdidos[indice_cruzamento]),
    xytext=(60, -40), textcoords="offset points", fontsize=8,
...
ax.annotate(
    f"limiar 0,5\n(FPR {fpr_05:.3f}, revocação {revocacao_05:.3f})",
    xy=(fpr_05, revocacao_05), xytext=(40, -45), textcoords="offset points", fontsize=8,
...
A matriz de confusão, a precisão, a revocação e a curva ROC não dependem de qual classificador as produziu: servem para LDA, QDA, Naive Bayes e a regressão logística do mesmo jeito, e é esse o vocabulário que a seção 9.6 usa para comparar os quatro lado a lado.
