...
O *k* de cada *k*-NN é fixo e declarado agora — `k_pequeno = 1` e `k_grande = 15` —, não ajustado ao próprio dado: cada valor de *k* é, na prática, um classificador diferente, e entra na tabela como tal, em vez de um só "*k*-NN".
...
Três cenários, cada um com dois preditores e duas classes com médias `media_a = (-1, 0)` e `media_b = (1, 0)` — só a primeira coordenada separa as classes, a segunda tem a mesma distribuição nas duas. O que muda de um cenário a outro é só a covariância dentro de cada classe: idêntica e sem correlação no primeiro, idêntica e com correlação 0,8 no segundo, com sinais opostos (0,7 e -0,7, a mesma diferença de covariância que a seção 9.4 usa, mas com as médias das classes mais próximas — aqui só a primeira coordenada as separa, em ±1,0, contra ±1,5 nas duas coordenadas da 9.4, daí o patamar de erro mais alto) no terceiro — e é só essa terceira mudança que torna a fronteira de Bayes curva em vez de reta. Duzentos sorteios de treino por cenário, cada um refazendo o ajuste dos seis classificadores contra o mesmo teste fixo daquele cenário.

```{python}
#| label: fig-comparacao-cenarios
#| fig-cap: "Distribuição do erro de teste em 200 sorteios de treino, para os seis classificadores, nos três cenários simulados (painéis, cada um com sua própria escala de erro). Em laranja, o classificador com menor erro médio naquele cenário; em azul, os demais. A cor marca só essa distinção; a identidade de cada classificador está no rótulo do eixo."
...
Dentro desse grupo apertado, uma comparação se sustenta: LDA erra menos que QDA em 81% dos 200 sorteios — uma vantagem sistemática, mas modesta, sobre uma margem estreita entre as médias (0,0054). O realce da figura anterior marca a menor média de cada painel; aqui, essa margem é estreita demais para separar os quatro. Entre LDA e Naive Bayes, a distância é menor ainda — pequena demais para separar os dois —, e o cenário seguinte mostra por que Naive Bayes consegue chegar tão perto aqui.
