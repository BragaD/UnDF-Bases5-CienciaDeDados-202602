...
Se acreditarmos que esses dois fatores são independentes, dá para preencher as quatro caixas sem nenhum dado: a fração de Lukes entre quem desenvolve leucemia é a mesma da população inteira. Aplicando o teste a um milhão de pessoas:
...
matriz["total"] = matriz.sum(axis=1)     # um total por linha
matriz.loc["total"] = matriz.sum()       # um total por coluna
...
Esta é a **matriz de confusão** das quatro caixas acima, agora com números. Ela é um `DataFrame` por um motivo prático: uma matriz de confusão sem rótulo é um convite ao erro — quatro números numa grade dois por dois, e nada dizendo qual é qual. Aqui a linha diz o que o modelo previu, a coluna diz qual era a verdade, e ninguém precisa lembrar de nada. É assim que uma matriz de confusão se apresenta: com a linha e a coluna dizendo o que são. O [Capítulo 9](../cap09/index.qmd) monta a dele com dados de verdade, não com uma conta.

A linha e a coluna `total` são a conferência: `matriz.sum(axis=1)` soma cada linha, `matriz.sum()` soma cada coluna, e é o mesmo `axis` das reduções de array do [bloco *De listas a arrays*](../cap07/01-explorando-seus-dados.qmd). Os totais fecham em um milhão, 14.000 doentes e 5.000 Lukes — que eram os dados do enunciado. `index=` e `columns=` são os rótulos que se escrevem na construção; e atribuir a um rótulo que ainda não existe — `matriz.loc["total"] = …` — acrescenta a linha em vez de sobrescrever alguma.

São os quatro números de dentro da tabela — `tp`, `fp`, `fn`, `tn`, nessa ordem — que as funções desta seção recebem. Com eles dá para calcular várias estatísticas de desempenho. A **acurácia** é definida como a fração de previsões corretas:
...
As quatro funções desta seção moram em `scratch_np.machine_learning`, junto com o `split_data` da [seção anterior](03-overfitting-e-underfitting.qmd) — o mesmo código que você acabou de escrever. É de lá que o [Capítulo 10](../cap10/index.qmd) e o [Capítulo 13](../cap13/index.qmd) as importam para avaliar os modelos deles.
...
Reconstruindo o milhão de pessoas do teste do Luke, os quatro números saem iguais aos que você calculou acima — o F1 difere no último bit de ponto flutuante, porque as duas contas combinam precisão e revocação em ordens diferentes. Não há truque nenhum nessas funções — elas contam as mesmas quatro caixas. O que muda é tudo o que fica implícito.

**O primeiro implícito é a orientação da matriz.** A tabela desta seção põe as previsões nas **linhas** — e diz isso, porque os rótulos estão escritos nela. O `confusion_matrix` do `scikit-learn` devolve um `ndarray` sem rótulo nenhum, com os valores **reais** nas linhas e as previsões nas colunas. É a transposta:
