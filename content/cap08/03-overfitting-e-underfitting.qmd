...
**A primeira coisa que ela esconde é a semente.** Este livro inteiro fixa `random.seed(N)`, e o nosso `split_data` obedece a isso porque usa `random.shuffle` da biblioteca padrão. O `scikit-learn` **não usa** o `random` da biblioteca padrão: ele tem o próprio gerador, controlado por `random_state`. Chamar `random.seed(42)` antes de `train_test_split` não torna a divisão reprodutível — duas chamadas seguidas devolvem conjuntos diferentes. Só `random_state=42` faz isso. E a regra vale para o pacote inteiro: todo objeto do `scikit-learn` que sorteia alguma coisa tem o seu próprio `random_state`, e nenhum deles escuta a semente que este livro fixa.
