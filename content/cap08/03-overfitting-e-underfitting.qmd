...
As duas funções ficam em `scratch_np.machine_learning`, exatamente como você as escreveu aqui — o módulo é o chunk salvo em arquivo. O [Capítulo 9](../cap09/index.qmd) importa `train_test_split` para dividir as flores do Iris — a matriz de medidas e o array de espécies, pelos mesmos índices —, o [Capítulo 10](../cap10/index.qmd) importa `split_data` para separar a lista de mensagens de e-mail, e o [Capítulo 13](../cap13/index.qmd) volta ao `train_test_split`. Nenhum deles reescreve a divisão.
...
## O que se divide, na prática, é uma tabela
...
assert treino.dtypes.equals(acoes.dtypes) and teste.dtypes.equals(acoes.dtypes)
assert set(treino.index).isdisjoint(teste.index)   # nada está nos dois lados
...
X_treino = treino[atributos].to_numpy()     # a fronteira, para cada lado
y_treino = treino["Close"].to_numpy()
X_teste = teste[atributos].to_numpy()
y_teste = teste["Close"].to_numpy()

X_treino.shape, X_treino.dtype, X_teste.shape, y_teste.shape
```

Quatro blocos de números, sem nome nenhum, prontos para o `@` — e é isto que o modelo veria se houvesse um modelo aqui. Não há: como na [seção 7.1](../cap07/01-explorando-seus-dados.qmd), este `X` e este `y` servem só para a forma, porque a máxima e a mínima do dia já contêm o fechamento e prever `y` a partir deles seria trapaça. E há um segundo motivo, que vale dizer em voz alta: pregões são uma **série temporal**, e uma série temporal não se divide por sorteio — divide-se por data, treinando no passado e testando no futuro. Aqui o sorteio serve para mostrar a mecânica da divisão, não para propor um experimento.

As duas ordens — dividir e depois atravessar, ou atravessar e depois dividir — dão o mesmo resultado para o modelo, e o livro usa as duas. Os capítulos [9](../cap09/index.qmd) e [13](../cap13/index.qmd) chamam `train_test_split` direto sobre `X` e `y`, porque ali `X` e `y` são tudo o que existe. Quando a tabela tem colunas que **não** são atributos — um identificador, uma data, o nome da ação —, dividir a tabela primeiro é o que preserva essas colunas para conferir depois.
