...
**A primeira coisa que ela esconde é a semente.** O nosso `split_data` usa `random.shuffle` da biblioteca padrão, então um `random.seed(42)` antes da chamada basta para tornar a divisão reprodutível. O `scikit-learn` **não usa** o `random` da biblioteca padrão: ele tem o próprio gerador, controlado por `random_state`. Chamar `random.seed(42)` antes de `train_test_split` não torna a divisão reprodutível — duas chamadas seguidas devolvem conjuntos diferentes. Só `random_state=42` faz isso. E a regra vale para o pacote inteiro: todo objeto do `scikit-learn` que sorteia alguma coisa tem o seu próprio `random_state`, e nenhum deles escuta o `random.seed` da biblioteca padrão.
