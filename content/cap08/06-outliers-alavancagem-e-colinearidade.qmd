...
A seção 8.5 comparou reta, parábola e polinômio de grau 5 só pelo R² que cada um explica de `milhas_por_galao`. R² resume o quanto um modelo erra; não diz se o erro que sobra tem um padrão que uma curva melhor teria capturado, se um único carro está puxando o ajuste sozinho, ou se dois preditores andam tão juntos que seus coeficientes deixaram de significar algo isolado. Esta seção olha para essas perguntas — o padrão que sobra no resíduo, o outlier, a alavancagem e a colinearidade —, e o instrumento comum às quatro é o gráfico: nenhuma delas aparece só olhando R².
...
round(r2_1, 2), round(rse1, 2), round(r2_2, 2), round(rse2, 2)
```
...
O `datsun 280-zx`, o pior resíduo, tem alavancagem 0,0066 — abaixo da média 0,0077: seu problema é só em y, o valor previsto para a sua `potencia`. Do outro lado, o `pontiac grand prix` tem a maior alavancagem do conjunto, 0,09, e `potencia_e_a_maior_do_conjunto` confirma por quê: sua `potencia`, 230 cv, é a mais alta entre os 392 carros. Mas seu resíduo padronizado é só 0,28, longe do corte de 3. Nenhum carro deste conjunto combina os dois problemas ao mesmo tempo — o ajuste não depende de um único ponto raro em x e mal explicado em y.
...
correlacoes_credit.round(2)
```

`limite` e `pontuacao` correlacionam a 1,00; `limite` e `idade`, a 0,10. Um sobe junto com o outro; o outro não tem relação visível com nenhum dos dois.
...
#| fig-cap: "Curvas de nível do RSS da regressão de saldo sobre dois preditores, para os coeficientes desses dois preditores — o intercepto fica fixo na média de saldo, e as curvas mostram o RSS minimizado só sobre os dois coeficientes do eixo. Esquerda: idade e limite, que quase não se correlacionam — o vale é uma elipse fechada e comparativamente redonda. Direita: pontuacao e limite, que correlacionam a 1,00 — o vale vira uma calha comprida e estreita, na diagonal: muitos pares (β_pontuacao, β_limite) quase empatam em RSS. O ponto marca o par que minimiza RSS em cada ajuste."
...
for ax, preditores, rotulos in [
    ...
    contornos = ax.contour(malha0, malha1, rss, levels=niveis, cmap="Blues")
...
def largura_beta_limite(preditores, eps=0.01):
    X = credit[preditores].to_numpy(dtype=float)
    Xc = X - X.mean(axis=0)
    yc = y_saldo - y_saldo.mean()
    M = Xc.T @ Xc
...
largura_idade_limite = largura_beta_limite(["idade", "limite"])
largura_pontuacao_limite = largura_beta_limite(["pontuacao", "limite"])
razao_larguras = largura_pontuacao_limite / largura_idade_limite

round(largura_idade_limite, 2), round(largura_pontuacao_limite, 2), round(razao_larguras, 2)
```

A largura mede a calha diretamente: fixando o outro coeficiente no valor que melhor cabe, a faixa de $\beta_{\text{limite}}$ que ainda deixa RSS a até 1% do mínimo mede 0,02 no par (idade, limite) e 0,25 no par (pontuacao, limite) — 12,70 vezes mais larga. É essa largura que a calha da figura desenha: quanto mais colinear o par, mais longe um novo dado poderia empurrar o par de coeficientes sem que RSS quase mudasse — e é isso que torna os coeficientes instáveis.

## Quatro problemas, um instrumento comum

Não linearidade, outlier, alavancagem e colinearidade não vêm sempre juntos. Neste capítulo, o U do resíduo linear tinha causa clara — faltava `potencia²` — e some ao corrigi-la; o outlier e a maior alavancagem recaíram sobre carros diferentes, nenhum com força para deslocar o ajuste sozinho; e foi só em `Credit`, com dois preditores que andam quase colados, que a colinearidade apareceu, alargando a faixa de coeficientes plausíveis em mais de doze vezes. Cada um pede o gráfico certo para se revelar — nenhum aparece só olhando R².
